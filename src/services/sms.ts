...
import { parseSmsListOutput, sanitizeSmsContent, parseRemovedSmsCount, sanitizePhoneNumber } from "../utils";
...
export const getOneMessage = async (index: number): Promise<SmsMessage | null> => {
...
export const sendMessage = async (number: string, content: string): Promise<void> => {
  const sanitizedNumber = sanitizePhoneNumber(number);
  const escapedContent = sanitizeSmsContent(content);
  
  await session.executeCommand(`sms send ${sanitizedNumber} "${escapedContent}"`);
};